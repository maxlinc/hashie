...
module Hashie
  ...
  class Hash < ::Hash
      ...
      keys.each do |k|
        ...
        if self[k].is_a?(Array)
          ...
          self[k].each do |array_object|
            out[assignment_key] << (Hash === array_object ? ___flexibly_convert_to_hash(array_object, options) : array_object)
          end
        else
          out[assignment_key] = (Hash === self[k] || self[k].respond_to?(:to_hash)) ? ___flexibly_convert_to_hash(self[k], options) : self[k]
        end
      end
      out
    end
    ...
    def ___flexibly_convert_to_hash(object, options = {})
